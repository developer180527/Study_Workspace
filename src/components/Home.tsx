    ...
    <div className="h-full flex flex-col bg-slate-50 overflow-y-auto">
      ...
      <header className="bg-white border-b border-slate-200 px-8 py-5 flex items-center justify-between shrink-0 top-0 sticky z-10 shadow-sm">
        ...
        <button 
          ...
          className="p-2.5 rounded-xl text-slate-500 bg-slate-100 hover:bg-slate-200:bg-slate-600 transition-colors"
          ...
          {workspaces.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {workspaces.map((ws) => (
                <div 
                  key={ws.id}
                  className="group bg-white border border-slate-200 rounded-2xl overflow-hidden hover:shadow-lg hover:border-indigo-300:border-indigo-500/50 transition-all cursor-pointer flex flex-col"
                ...
                >
                    ...
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          ...
                          className="p-1.5 text-slate-400 hover:text-indigo-600:text-indigo-400 bg-slate-50 rounded-md transition-colors"
                        ...
                        <button
                          ...
                          className="p-1.5 text-slate-400 hover:text-rose-600:text-rose-400 bg-slate-50 rounded-md transition-colors"
                  ...
                  <div className="bg-slate-50 p-4 border-t border-slate-100 flex justify-between items-center group/btn" onClick={() => onLaunchWorkspace(ws.id)}>
                    <span className="text-sm font-semibold text-slate-600 group-hover/btn:text-indigo-600:text-indigo-400 flex items-center gap-1.5 transition-colors">
